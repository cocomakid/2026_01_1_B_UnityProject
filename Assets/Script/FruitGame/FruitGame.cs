...
public class FruitGame : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && fruitTimer == -3.0f)             // 마우스 좌 클릭하면 과일을 떨어뜨린다 && 타이머 시간은 -0.3f
        {
            DropFruit();
        }
    ...
    void SpawnNewFruit()                                // 과일 생성 함수
    {
        if (!isGameOver)                                // 게임 오버가 아닐 때만 새 과일 생성
        {
            ...
            if (rb != null)
            {
                rb.gravityScale = 0f;                       // 시작 시에는 중력 스케일 0으로 적용
            }
        }
    }
    ...
    void DropFruit()
    {
        ...
        if(rb != null)
        {
            ...
            currentFruit = null;                    // 현재 들고 잇는 과일 해제
            fruitTimer = 1.0f;                        // 타이머 초기화
        }
    }

    public void MergeFruits(int fruitType, Vector3 posiotion)
    {
        if (fruitType < fruitPrefabs.Length -1)             // 마지막 과일 타입이 아니라면
        {
            GameObject newFruit = Instantiate(fruitPrefabs[fruitType +1], posiotion, Quaternion.identity);              // 해당 타입 다음 번호를 생성 시킨다
